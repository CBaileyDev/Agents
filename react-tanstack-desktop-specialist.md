...
- **Radix primitives**: `Dialog`, `AlertDialog`, `DropdownMenu`, `ContextMenu` (essential for right-click), `ScrollArea` (a11y scrollbars), `Popover`, `Tooltip`, `Select`. **As of shadcn Feb 2026** the new-york style migrated to the unified `radix-ui` package; old `@radix-ui/react-*` still works but is legacy. shadcn also supports Base UI as alternate backend
...
- shadcn migration (Feb 2026) to the `radix-ui` umbrella package: existing components still work, but new components from `npx shadcn add` use the new import path — mixed apps will have both
